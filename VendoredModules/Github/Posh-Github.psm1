  ...
  $Creator, $Mentioned, $Labels, $Sort, $Direction, $Since)
{
  ...
  Write-Verbose "Requesting issues for $Owner/$Repository"
...
function GetIssueComments($RootURL = '',$Owner, $Repository, $Issue, $Sort, $Direction, $Since)
{
  #TODO: follow link headers
  $sinceParam, $labelsParam = '', ''
  if ($Since)
  ...
  if ($RootURL -ne '') {
    ...
    $uri = ("https://api.github.com/repos/$Owner/$Repository/issues/$Issue/" +
    "?state=$state$Milestone$Assignee$Creator$Mentioned" +
    "$labelsParam&sort=$Sort&direction=$Direction$Since" +
    "&access_token=${Env:\GITHUB_OAUTH_TOKEN}")
  }
  ...
  Write-Verbose "Requesting your issue comments"
  $global:GITHUB_API_OUTPUT = Invoke-RestMethod -Uri $uri
  #Write-Verbose $global:GITHUB_API_OUTPUT

  $global:GITHUB_API_OUTPUT | % { Write-Output $_ }
}
...
function Get-GitHubIssueComments
{
  ...
  param(
    ...
    [Parameter(Mandatory = $True, Position=2, ParameterSetName='repo')]
    [string]
    $IssueID = $null,

    [Parameter(Mandatory = $true, Position=2, ParameterSetName='issueobject')]
    [Object]$GithubIssue,
  ...
  try
  {
    ...
    switch ($PsCmdlet.ParameterSetName)
    {
      'repo'
      {
        ...
        if ([string]::IsNullOrEmpty($IssueID)) { Throw "Requires an Issue ID" }
        ...
        GetIssueComments '' $Owner $Repository $IssueID $Sort.ToLower() $Direction.ToLower() $PsBoundParameters.Since
      }
      'issueobject' {
        GetIssueComments $GithubIssue.comments_url '' '' '' $Sort.ToLower() $Direction.ToLower() $PsBoundParameters.Since
      }
    }
  }
  catch
